...
/**
 * JPA Repository for ReadingLog entity operations. Provides:
 * 
 * - Custom query methods for:
 *   * User-specific log retrieval (by ID, date ranges, time spent)
 *   * Multi-field search (title, author, notes)
 *   * Duplicate detection (title/author uniqueness checks)
 *   * Version history tracking (previousVersion)
 *   * Combined filtering (date + time ranges)
 * 
 * @author Jianghui Sun
 * @author Yiting Jiang
 * @author Qiyu Cao
